...
class TestMoveItems(IntegrationTestCase):
    ...
    def test_only_open_items_appear_in_destination_widget(self):
        ...
        self.assertNotIn(IUUID(self.resolved_dossier), uids,
...
class TestMoveItemsWithTestbrowser(IntegrationTestCase):
    ...
    def test_document_inside_closed_dossier_is_not_movable(self, browser):
        self.login(self.dossier_manager)
        self.move_items(
            browser, src=self.resolved_dossier,
            obj=self.resolved_document, target=self.empty_dossier)

        self.assertEqual(
            ['Can only move objects from open dossiers!'],
            error_messages())
        self.assertIn(self.resolved_document,
                      self.resolved_dossier.objectValues())
        self.assertNotIn(self.resolved_document,
    ...
    def test_task_inside_closed_dossier_is_not_movable(self, browser):
        ...
        self.move_items(
            browser, src=self.resolved_dossier,
            task=self.resolved_task, target=self.empty_dossier)

        self.assertEqual(
            ['Can only move objects from open dossiers!'],
            error_messages())
        self.assertIn(self.resolved_task, self.resolved_dossier.objectValues())
        self.assertNotIn(self.resolved_task, self.empty_dossier.objectValues())
    ...
    def test_paste_action_not_visible_for_closed_dossiers(self, browser):
        ...
        browser.open(self.resolved_dossier)
        self.assertIsNone(
            browser.find('Paste'),
