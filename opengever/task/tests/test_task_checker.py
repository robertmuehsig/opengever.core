...
class TestTaskControllerChecker(IntegrationTestCase):
    ...
    def test_all_subtasks_finished_is_allways_true_when_no_subtask_exists(self):
        self.login(self.dossier_responsible)
        self.assertTrue(get_checker(self.resolved_task).task.all_subtasks_finished)
    ...
    def test_all_subtasks_finished_if_predecessor_has_subtasks(self):
        ...
        self.set_workflow_state('task-state-in-progress', self.resolved_task)
        self.register_successor(self.task, self.resolved_task)

        task_checker = TaskChecker(self.resolved_task.get_sql_object())
