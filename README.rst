...
Once your test is finished you should tear down and re-setup for the next test in order to isolate the databse properly:
...
The testserver sets up a ``service.user`` which has a REST-API service key and is allowed to impersonate other user.
...
When developing third party application, it is best practice to use a tape recording system.
In local development, a real testserver should be started and tapes of it's responses should be recorded.
...
of the testserver, so that compatibility with the new version can be proofen.
